...
std::vector<rt::float16> open_float16_bin(const std::string& filename) {
    ...
    // float 개수 계산 (4바이트당 하나)
    size_t num_elements = size_in_bytes / sizeof(rt::float16);

    // 벡터에 읽기
    std::vector<rt::float16> data(num_elements);
    if (!file.read(reinterpret_cast<char*>(data.data()), size_in_bytes)) {
        std::cerr << "파일 읽기 실패\n";
        return {};
    }
    return data;
...
int main(int argc, char ** argv){
            ...
            if (arg == "--n_thread" || arg == "-t") {
                ...
                if (i + 1 < argc && argv[i + 1][0] != '-') {
                    ...
                    if (input_audio_ctx <= 0) {
                        std::cerr << "error: invalid audio_context '%s'\n", argv[i + 1];
                        return -1;
                    }
                } else {
    ...
    EncOptions.DisableDenormals = false;
    rt::ModelLoadOptions CrOptions;
    CrOptions.ThreadCount = n_thread;
    CrOptions.DisableDenormals = false;
    ...
    if (whisper_pcm_to_mel_with_state(ctx, ctx->state, samples, n_samples, n_thread) != 0) {
    ...
    while(true){
        ...
        std::unique_ptr<rt::float16[]> cc01, cc03, cc11, cc13, cc21, cc23, cc31, cc33, cc41, cc43, cc51, cc53;
        std::unique_ptr<rt::float16[]> cc61, cc63, cc71, cc73, cc81, cc83, cc91, cc93, cc101, cc103, cc111, cc113;
        ...
        rt::TypedTensor<rt::float16> tns_cc01, tns_cc03, tns_cc11, tns_cc13, tns_cc21, tns_cc23, tns_cc31, tns_cc33, tns_cc41, tns_cc43, tns_cc51, tns_cc53;
        rt::TypedTensor<rt::float16> tns_cc61, tns_cc63, tns_cc71, tns_cc73, tns_cc81, tns_cc83, tns_cc91, tns_cc93, tns_cc101, tns_cc103, tns_cc111, tns_cc113;
        ...
        auto f16_start = std::chrono::high_resolution_clock::now();
        auto indata_f16 = std::make_unique<rt::float16[]>(80 * 2 * n_audio_ctx);
        float* indata_pointer = indata.get();
        for(int i = 0; i < 80 * 2 * n_audio_ctx; ++i){
            indata_f16[i] = static_cast<rt::float16>(indata[i]);
        }
        auto f16_end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> f16_ms = f16_end - f16_start;
        std::cout << "f16 convert time: " << f16_ms.count() << " ms\n";
	...
	auto compute_start = std::chrono::high_resolution_clock::now();
        ...
        // run encoder
        {
            ...
            rt::TypedTensor<rt::float16> enc_input = rt::tensor<rt::float16>({1, 80, (unsigned int)2*n_audio_ctx}, indata_f16.get()); // TODO - jy - shape을 transpose해야 함. 임시 shape
            std::vector<rt::Tensor> enc_inputs;
            enc_inputs.emplace_back(enc_input);
            auto encoutdata = std::make_unique<rt::float16[]>(n_audio_ctx*n_audio_state);
            rt::TypedTensor<rt::float16> enc_output = rt::tensor<rt::float16>({1,(unsigned int)n_audio_ctx, n_audio_state}, encoutdata.get());
            ...
            cc01 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);
            cc02 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc03 = std::make_unique<rt::float16[]>(n_text_state);
            cc04 = std::make_unique<int[]>(n_text_state);
            tns_cc00 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc00.get());  
            tns_cc01 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc01.get());  
            tns_cc02 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc02.get());
            tns_cc03 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc03.get()); 
            tns_cc04 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc04.get());
            // cross cache 1
            cc10 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc11 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);
            cc12 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc13 = std::make_unique<rt::float16[]>(n_text_state);
            cc14 = std::make_unique<int[]>(n_text_state);
            tns_cc10 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc10.get());  
            tns_cc11 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc11.get());  
            tns_cc12 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc12.get());
            tns_cc13 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc13.get()); 
            tns_cc14 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc14.get());
            // cross cache 2
            cc20 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc21 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc22 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc23 = std::make_unique<rt::float16[]>(n_text_state);
            cc24 = std::make_unique<int[]>(n_text_state);
            tns_cc20 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc20.get());  
            tns_cc21 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc21.get());  
            tns_cc22 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc22.get());
            tns_cc23 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc23.get()); 
            tns_cc24 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc24.get());
            // cross cache 3
            cc30 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc31 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc32 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc33 = std::make_unique<rt::float16[]>(n_text_state);
            cc34 = std::make_unique<int[]>(n_text_state);
            tns_cc30 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc30.get());
            tns_cc31 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc31.get());
            tns_cc32 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc32.get());
            tns_cc33 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc33.get()); 
            tns_cc34 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc34.get());
            // cross cache 4
            cc40 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc41 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc42 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc43 = std::make_unique<rt::float16[]>(n_text_state);
            cc44 = std::make_unique<int[]>(n_text_state);
            tns_cc40 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc40.get());
            tns_cc41 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc41.get());
            tns_cc42 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc42.get());
            tns_cc43 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc43.get());
            tns_cc44 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc44.get());
            // cross cache 5
            cc50 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc51 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc52 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc53 = std::make_unique<rt::float16[]>(n_text_state);
            cc54 = std::make_unique<int[]>(n_text_state);
            tns_cc50 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc50.get());
            tns_cc51 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc51.get());
            tns_cc52 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc52.get());
            tns_cc53 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc53.get());
            tns_cc54 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc54.get());
            /*
            // cross cache 6
            cc60 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc61 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc62 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc63 = std::make_unique<rt::float16[]>(n_text_state);
            cc64 = std::make_unique<int[]>(n_text_state);
            tns_cc60 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc60.get());
            tns_cc61 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc61.get());
            tns_cc62 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc62.get());
            tns_cc63 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc63.get());
            tns_cc64 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc64.get());
            // cross cache 7
            cc70 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc71 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc72 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc73 = std::make_unique<rt::float16[]>(n_text_state);
            cc74 = std::make_unique<int[]>(n_text_state);
            tns_cc70 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc70.get());
            tns_cc71 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc71.get());
            tns_cc72 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc72.get());
            tns_cc73 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc73.get()); 
            tns_cc74 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc74.get());
            // cross cache 8
            cc80 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc81 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc82 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc83 = std::make_unique<rt::float16[]>(n_text_state);
            cc84 = std::make_unique<int[]>(n_text_state);
            tns_cc80 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc80.get());
            tns_cc81 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc81.get());
            tns_cc82 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc82.get());
            tns_cc83 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc83.get()); 
            tns_cc84 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc84.get());
            // cross cache 9
            cc90 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc91 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc92 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc93 = std::make_unique<rt::float16[]>(n_text_state);
            cc94 = std::make_unique<int[]>(n_text_state);
            tns_cc90 = rt::tensor<int8_t>({1,n_text_head,n_audio_ctx,n_text_state_per_head}, cc90.get());
            tns_cc91 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc91.get());
            tns_cc92 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc92.get());
            tns_cc93 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc93.get());
            tns_cc94 = rt::tensor<int>({1,n_text_head,n_text_state_per_head}, cc94.get());
            // cross cache 10
            cc100 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc101 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc102 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc103 = std::make_unique<rt::float16[]>(n_text_state);
            ...
            tns_cc101 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc101.get());
            tns_cc102 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc102.get());
            tns_cc103 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc103.get());
            ...
            cc111 = std::make_unique<rt::float16[]>(n_text_head * n_audio_ctx);  
            cc112 = std::make_unique<int8_t[]>(n_text_state * n_audio_ctx);
            cc113 = std::make_unique<rt::float16[]>(n_text_state);
            ...
            tns_cc111 = rt::tensor<rt::float16>({1,n_text_head,n_audio_ctx,1}, cc111.get());
            tns_cc112 = rt::tensor<int8_t>({1, n_text_head, n_audio_ctx * n_text_state_per_head }, cc112.get());
            tns_cc113 = rt::tensor<rt::float16>({1,n_text_head,1,n_text_state_per_head}, cc113.get());
        ...
        std::unique_ptr<rt::float16[]> logit_out_f16(new rt::float16[ctx->vocab.n_vocab]);
        ...
        */
        rt::TypedTensor<rt::float16> dec_output = rt::tensor<rt::float16>({1,1,(unsigned int) ctx->vocab.n_vocab}, logit_out_f16.get());
        ...
        std::cout << "Decoder Request time : " << dec_req_ms.count() << " ms\n";
            ...
            for(int it=0; it < (int) temperatures.size(); ++it){
                ...
                // init prompt and kv cache for the current iteration
                {
                    ...
                    rt::float16* logit_out_f16_data = logit_out_f16.get();
                    for(int i = 0; i < n_logits; ++i){
                        logit_out[i] = static_cast<float>(logit_out_f16_data[i]);
                    }
                            ...
                            while (true) {
                                //GREEDY
                                {
                                    if (t_cur < 1e-6f) {
                                        ...
                                        sequence->tokens.push_back(whisper_sample_token(*ctx, probs, logprobs, false));
                                    }
            ...
            std::chrono::duration<double, std::milli> compute_ms = compute_end - compute_start;
            std::cout << "Total compute time : " << compute_ms.count() << " ms\n";
